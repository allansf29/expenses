...
const handleExportCSV = useCallback(
  (mode: "month" | "all" = "month") => {
    ...
    // Se for modo "month", filtra apenas o mês/ano selecionado
    if (mode === "month") {
      ...
      if (filteredExpenses.length === 0) {
        console.error("Não há lançamentos neste mês para exportar.");
        return;
      }
    }
    ...
    filteredExpenses.forEach((e) => {
      const id = e.id;
      const dateStr = format(new Date(e.date), "dd/MM/yyyy");
    ...
    const fileName =
      mode === "month"
        ? `financas_${format(selectedDate, "MM-yyyy")}.csv`
        : `financas_completo.csv`;
