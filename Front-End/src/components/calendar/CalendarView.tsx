...
const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  // O renderHeader agora usa as props prevMonth e nextMonth
  const renderHeader = () => (
    <div className="flex items-center justify-between mb-2 pb-4 border-b border-gray-800 text-white">
      <div className="flex items-center gap-2">
        {/* CORREÇÃO: Usando prevMonth e nextMonth (funções prontas do hook) */}
        <SimpleButton className="h-8 w-8 text-gray-400 hover:bg-gray-700 bg-transparent hover:text-white p-0 focus:ring-gray-600" onClick={prevMonth}>{"<"}</SimpleButton>
        <h2 className="text-xl font-medium w-48 text-left capitalize">{format(currentMonth, "MMMM yyyy", { locale: ptBR })}</h2>
        <SimpleButton className="h-8 w-8 text-gray-400 hover:bg-gray-700 bg-transparent hover:text-white p-0 focus:ring-gray-600" onClick={nextMonth}>{">"}</SimpleButton>
      </div>
